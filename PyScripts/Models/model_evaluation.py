...
def get_final_metrics(model_obj, X_train, y_train, X_test, y_test, n_splits: int =3):
    ...
    cv_results=cross_validate(model_obj, X_train, y_train, cv=tscv, return_train_score=True, n_jobs=-1)

    mean_train=np.mean(cv_results['train_score'])
    std_train=np.std(cv_results['train_score'])

    mean_cv_test=np.mean(cv_results['test_score'])
    std_cv_test=np.std(cv_results['test_score'])
