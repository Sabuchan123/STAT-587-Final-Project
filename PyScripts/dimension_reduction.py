...
def step_wise_reg_cv(X_train: pd.DataFrame, y_train: pd.DataFrame, X_test: pd.DataFrame, model) -> tuple[pd.DataFrame, pd.DataFrame]:
    print("------- Applying Step-Wise Regression")
    X_train_index=X_train.index
    ...
    print("Finished Applying Step-Wise Regression -------")
    return X_train[selected_features], X_test[selected_features]

def step_wise_reg_wfv(X_train: pd.DataFrame, y_train: pd.DataFrame, X_test: pd.DataFrame, model) -> tuple[pd.DataFrame, pd.DataFrame]:
    tscv=TimeSeriesSplit(n_splits=3)
    mfs=MFS(model, int(np.sqrt(X_train.shape[0])), forward=True, floating=True, cv=tscv, n_jobs=-1)
    mfs.fit(X_train, y_train)
    selected_features=list(mfs.k_feature_names_)
    return X_train[selected_features], X_test[selected_features]

def LASSO(X_train: pd.DataFrame, y_train: pd.DataFrame) -> pd.DataFrame:
    lasso_cv=LassoCV(cv=4, random_state=42, n_jobs=-1)
    lasso_cv.fit(X_train, y_train)
    return X_train[X_train.columns[lasso_cv.coef_!=0]]

def RIDGE(X_train: pd.DataFrame, y_train: pd.DataFrame, n_features: int):
    scaler=StandardScaler()
    X_train=scaler.fit_transform(X_train)
    ...
    return X_train[X_train.columns[top_stocks]]